...
/// Implemented by triangles that can perform barycentric interpolation
/// of values derived from its vertices.
pub trait Interpolation: Triangle {
    ...
    /*/// Interpolates a new vertex at the given world-space position from the vertices of the triangle.
    fn interpolate_vertex_at(&self, position: Vec3) -> Self::Vertex
        where Self::Vertex: Clone + Mul<f32, Output = Self::Vertex> + Add<Self::Vertex, Output = Self::Vertex>{
        self.interpolate_vertex_bary(self.barycentric_at(position))
    }

    /// Interpolates or extrapolates a new vertex at the given position in baricentric coordinates.
...
/// Implemented by triangles that can interpolate and extrapolate vertices on their surface.
...
impl<T: Triangle> InterpolateVertex for T
where
    Self::Vertex:
        Clone + Mul<f32, Output = Self::Vertex> + Add<Self::Vertex, Output = Self::Vertex>,
{
    fn interpolate_vertex_bary(&self, bary: [f32; 3]) -> Self::Vertex
    where
        Self::Vertex:
...
mod test {
    ...
    fn interpolate_vertex() {
        ...
        assert_ulps_eq!(point_on_there, tri.interpolate_vertex_at(point_on_there));
    }
    ...
    fn interpolate_owned_vertex() {
        ...
        assert_ulps_eq!(middle_vertex.position, on_there);

        // Expected texcoords interpolated at centroid to be 0.5/0.5 in the example
        assert_ulps_eq!(middle_vertex.texcoords, Vec2::new(0.5, 0.5));

        // Expected normal to be unchanged since they are uniform on the triangle
        assert_ulps_eq!(middle_vertex.normal, Vec3::new(0.0, 0.0, 1.0));
    }
}
